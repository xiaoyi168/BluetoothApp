...
public class SoftPumpActivity extends BaseActivity implements View.OnClickListener {

	private final static String TAG = SoftPumpActivity.class.getSimpleName();

	private ImageView iv_soft_icon;
	private Button btn_soft_standby;
	private TextView tv_soft_state;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_pump_soft);
		...
		iv_soft_icon.setImageResource(icon);
	...
	private void initUI(){
		iv_soft_icon = findViewById(R.id.iv_soft_icon);
		btn_soft_standby = findViewById(R.id.btn_soft_standby);
		tv_soft_state = findViewById(R.id.tv_soft_state);

		btn_soft_standby.setOnClickListener(this);
	}
...
//			finish();
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
		...
		if (gattServices.size() > 0 && mBluetoothLeService.get_connected_status(gattServices) >= 4) {
			if (connect_status_bit) {
				...
				Toast toast = Toast.makeText(SoftPumpActivity.this, "设备没有连接！", Toast.LENGTH_SHORT);
				toast.show();
			}
		}
	}
	...
	private void updateConnectionState(final int resourceId) {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				tv_soft_state.setText(resourceId);
			}
		});
	}
