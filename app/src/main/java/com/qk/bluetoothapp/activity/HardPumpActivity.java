...
public class HardPumpActivity extends BaseActivity implements View.OnClickListener {

	private final static String TAG = HardPumpActivity.class.getSimpleName();

	private ImageView iv_hard_icon;
    private Button btn_hard_standby;
	private TextView tv_hard_state;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_pump_hard);
        ...
        iv_hard_icon.setImageResource(icon);
	...
	private void initUI(){
		iv_hard_icon = findViewById(R.id.iv_hard_icon);
		btn_hard_standby = findViewById(R.id.btn_hard_standby);
		tv_hard_state = findViewById(R.id.tv_hard_state);

		btn_hard_standby.setOnClickListener(this);
	}
...
//			finish();
	...
	private void displayGattServices(List<BluetoothGattService> gattServices) {
		...
		if (gattServices.size() > 0 && mBluetoothLeService.get_connected_status(gattServices) >= 4) {
			if (connect_status_bit) {
				...
				Toast toast = Toast.makeText(HardPumpActivity.this, "设备没有连接！", Toast.LENGTH_SHORT);
				toast.show();
			}
		}
	}
	...
	private void updateConnectionState(final int resourceId) {
		runOnUiThread(new Runnable() {
			@Override
			public void run() {
				tv_hard_state.setText(resourceId);
			}
		});
	}
